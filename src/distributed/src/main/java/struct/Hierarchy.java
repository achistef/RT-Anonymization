...
public class Hierarchy implements Serializable
{
	...
	private final HashMap<Integer, Double> leavesMap;
	private final HashMap<Integer, BigInteger> leavesMapExp;
	...
	private int calcLeaves(Integer node)
	{
		...
		this.leavesMap.put(node, (double)sum/leaves);
		this.leavesMapExp.put(node, TWO.pow(sum).subtract(BigInteger.ONE));
		return sum == 0 ? 1 : sum;
	}
	...
	public double numOfLeaves(Integer node)
	{
		return this.leavesMap.get(node);
	}

	public BigInteger numOfLeavesExp(Integer node)
	{
		return this.leavesMapExp.get(node);
	}
